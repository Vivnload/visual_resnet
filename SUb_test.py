...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    for i, (image_tensors, labels) in enumerate(evaluation_loader):
        ...
        if 'CTC' in opt.Prediction:
            ...
            predss = model(image, text_for_pred, is_train=False)
            forward_time = time.time() - start_time
            ...
            for gt, pred in zip(labels, all_pred_strs):
                gt = gt[:gt.find('[s]')]
        ...
        return valid_loss_avg.val(), accuracy, norm_ED, all_pred_strs, all_confidence_scores, labels, infer_time, length_of_data
